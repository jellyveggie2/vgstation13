	...
	if(charging)
		...
		if(istype(charging, /obj/item/weapon/gun/energy)) // Original values: 100e charged, 150e wasted,
			...
			if((E.power_supply.charge + charge_unit) < E.power_supply.maxcharge)
				...
				if(!self_powered)
					machine_power_load += active_power_usage * (charge_unit + 150 * efficiency_modifier * charging_speed_modifier)
				update_icon()
			else
			...
			if((M.bullets + charge_unit) < M.max_bullets)
				...
				icon_state = "recharger1"
				if(!self_powered)
					machine_power_load += active_power_usage * (150 * charging_speed_modifier + 100 * efficiency_modifier * charging_speed_modifier)
				update_icon()
			else
			...
			if(B.bcell)
				if(B.bcell.give(175*charging_speed_modifier))
					icon_state = "recharger1"
					if(!self_powered)
						machine_power_load += active_power_usage * (200 * charging_speed_modifier)
				else
					icon_state = "recharger2"
					if(!has_beeped)
						playsound(src, 'sound/machines/charge_finish.ogg', 50)
...
/obj/machinery/recharger/proc/try_use_power(var/amount)
	if(self_powered)
		return
	machine_power_load += active_power_usage * amount
	...
	if(charging)
		if(istype(charging, /obj/item/weapon/gun/energy))
			...
			if(E.power_supply.charge < E.power_supply.maxcharge)
				...
				if(!self_powered)
					machine_power_load += active_power_usage * 250
			else
		...
		if(istype(charging, /obj/item/weapon/melee/baton))
			...
			if(B.bcell)
				if(B.bcell.give(175))
					icon_state = "wrecharger1"
					if(!self_powered)
						machine_power_load += active_power_usage * 200
				else
					icon_state = "wrecharger2"
			else
