			...
			if(!powmonitor)
				...
				if(L.len > 0)
					...
					for(var/obj/machinery/power/apc/A in L)
						...
						menu += " [S[A.equipment+1]] [S[A.lighting+1]] [S[A.environ+1]] [add_lspace(A.lastused_total.apparent_power(), 6)]  [A.cell ? "[add_lspace(round(A.cell.percent()), 3)]% [chg[A.charging+1]]" : "  N/C"]<BR>"
