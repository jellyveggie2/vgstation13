...
/obj/structure/cable
	...
	var/oldavail = 0
	var/oldnewavail = 0
	var/datum/power_vector/oldload = new()
...
/obj/structure/cable/proc/report_load(mob/user)
	if((powernet) && (powernet.avail > 0))		// is it powered?
		to_chat(user, "<SPAN CLASS='warning'>Power network status report - Load: [format_watts(powernet.load.apparent_power())] - Available: [format_watts(powernet.avail)].</SPAN>")
	else
...
/obj/structure/cable/proc/add_load(var/datum/power_vector/load)
	if(get_powernet())
		powernet.load += load
