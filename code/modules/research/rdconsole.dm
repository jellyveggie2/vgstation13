...
/obj/machinery/computer/rdconsole/proc/deconstruct_item(mob/user)
	...
	spawn(24)
		if(linked_destroy)
			...
			machine_power_usage += linked_destroy.active_power_usage
			screen = 1.0
			updateUsrDialog()
			linked_destroy.busy = 0
	...
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
		...
		if(linked_lathe)
			...
			if(being_built)
				...
				for(var/i=1;i<=n;i++)
					linked_lathe.machine_power_usage += linked_lathe.active_power_usage.unit() * power
		...
		if(linked_imprinter)
			...
			if(being_built)
				...
				for(var/i=1;i<=n;i++)
					...
					linked_imprinter.machine_power_usage += power * linked_imprinter.active_power_usage.unit()
				if(href_list["now"]=="1")
